...
window.state = {
    ...
    // Single shared date range used by BOTH the Dashboard and Reports views.
    // Do not add any other date-range state (no separate revenueRange /
    // reportDateFrom / reportDateTo) — everything reads/writes this one object.
    financialDateRange: { from: null, to: null },
    categories: [],
};

// Set the default financial date range immediately (function declarations are
// hoisted, so getDefaultRevenueRange() is safe to call here even though it's
// defined further down the file). This guarantees state.financialDateRange is
// always populated before any async Firestore snapshot or DOMContentLoaded
// callback can trigger renderDashboard()/renderReports().
state.financialDateRange = getDefaultRevenueRange();
...
// ============================================================
// 3b. PURE CANVAS CHART — z.html style (used for Gross Profit)
// ============================================================
function drawZLineChart(canvasId, data, lineColor, fillTop, fillBot) {
    ...
    var hasData = data && data.length > 0 && data.some(function(v) { return v !== 0 && v !== null && v !== undefined; });
    if (!hasData) {
    ...
    function drawCurve() {
        ...
        for (var i = 0; i < pts.length - 1; i++) {
            var xm = (pts[i].x + pts[i + 1].x) / 2;
            var ym = (pts[i].y + pts[i + 1].y) / 2;
            if (i === 0) {
...
function switchView(view) {
    ...
    document.querySelectorAll(".sidebar-nav .nav-link").forEach(l => l.classList.toggle("active", l.dataset.view === view));
...
function applyRoleVisibility(role) {
    ...
    document.querySelectorAll(".manager-only").forEach(el => { el.style.display = (role === "admin" || role ===
            "manager") ? "" : "none"; });
}
...
function attachRealtimeListeners() {
    ...
    state.unsubscribers.push(
        db.collection("products").orderBy("name").onSnapshot(snap => {
            ...
            // renderReports() also refreshes the investment breakdown, so
            // this keeps Dashboard KPI, Reports KPI, and Reports Details
            // all reading the same numbers no matter which listener fired.
            renderReports();
        }, err => showToast("Products sync error: " + err.message, "error"))
    );

    // Sales – only managers+ can view sales history
    if (hasRole("manager")) {
        state.unsubscribers.push(
    ...
    // Stock logs — calcPeriodInvestment() depends on state.stockLogs, and
    // Products/Sales frequently finish loading before Stock Logs does.
    // Re-rendering Dashboard and Reports here closes that race: whichever
    // collection loads last, both views end up showing the correct figure.
    if (hasRole("manager")) {
        state.unsubscribers.push(
...
// ============================================================
// 8. DASHBOARD (with date filter, line chart, gross profit)
// ============================================================
...
// Resets the ONE shared date range used by both Dashboard and Reports, syncs
// both sets of date inputs, and leaves it to the caller to re-render. Both
// the Dashboard "Reset" button and the Reports "Reset" button call this same
...
// Current, un-filtered value of all inventory on hand (cost * stock). This is
// a different metric than "investment over a period" — it's kept only as a
// general-purpose utility and must NEVER be called from renderDashboard(),
// renderReports(), or renderInvestmentBreakdown(). Those always use
// calcPeriodInvestment() against the single shared state.financialDateRange
// so the KPI numbers can never disagree with each other again.
function calcInvestment() {
    return state.products.reduce((sum, p) => sum + (p.cost || 0) * (p.stock || 0), 0);
}

function calcTotalRevenue() {
    try {
        return state.sales.filter(s => !s.voided && !s.refunded).reduce((sum, s) => sum + (s.total || 0), 0);
    } catch (e) { console.warn("calcTotalRevenue error:", e); return 0; }
}

// The ONLY function that may compute "investment" for display. Dashboard KPI,
// Reports KPI, and Reports Details all call this with the same
// state.financialDateRange, so they always agree.
...
function renderGrossProfitChartCanvas() {
    ...
    var emptyEl = document.getElementById("grossProfitChartEmpty");
    var totalEl = document.getElementById("gpChartTotal");
    ...
    try {
        ...
        if (!from || !to) {
            var def = getDefaultRevenueRange();
            from = def.from;
            to = def.to;
        }

        var salesInRange = state.sales.filter(function(s) {
            if (s.voided || s.refunded) return false;
            if (!s.timestamp || typeof s.timestamp.toDate !== 'function') return false;
            var t = s.timestamp.toDate();
            return t >= from && t <= to;
        });
        ...
        salesInRange.forEach(function(s) {
            var t = s.timestamp.toDate();
            var key = t.toISOString().split('T')[0];
            if (dayProfit[key] !== undefined) {
                var revenue = s.total || 0;
                var cogs = (s.items || []).reduce(function(sum, item) { return sum + (item.cost || 0) * (item.qty || 0); }, 0);
        ...
        var labels = Object.keys(dayProfit).sort();
        var profits = labels.map(function(k) { return Number(dayProfit[k].toFixed(2)); });
        var hasData = profits.some(function(v) { return v !== 0; });
        ...
        var dayLabels = labels.map(function(d) {
            var date = new Date(d + 'T00:00:00');
            return dayNames[date.getDay() === 0 ? 6 : date.getDay() - 1];
        });

        var labelContainer = document.getElementById("gpDayLabels");
        if (labelContainer) {
            labelContainer.innerHTML = dayLabels.map(function(name) {
                return '<span>' + name + '</span>';
            }).join('');
        }

        var lineColor = '#4ADE80';
        var fillTop = 'rgba(74,222,128,0.25)';
        var fillBot = 'rgba(74,222,128,0.0)';
...
function renderGrossProfitKpi() {
    ...
    try {
        ...
        salesInRange.forEach(s => {
            ...
            const cogs = (s.items || []).reduce((sum, item) => sum + (item.cost || 0) * (item.qty || 0), 0);
            totalCogs += cogs;
        ...
        // Damaged inventory is a real cost, so it must reduce gross profit —
        // not just show up in the separate "Total Loss" figure.
...
function renderStockMovementSummary() {
    ...
    try {
        ...
        if (filtered.length === 0) {
            container.innerHTML = `<div class="empty-state py-3"><i class="bi bi-arrow-left-right"></i> No stock movements in this period.</div>`;
            return;
        }
        const inTotal = filtered.filter(l => l.type === "in").reduce((s, l) => s + l.qty, 0);
        const outTotal = filtered.filter(l => l.type === "out" || l.type === "damage").reduce((s, l) => s + l.qty, 0);
...
function addToCart(productId) {
    try {
        ...
        if (existing) {
            ...
            state.cart.push({ productId, name: product.name, sku: product.sku, price: product.price, cost: product
...
async function completeSale() {
    ...
    const discountRaw = parseFloat(document.getElementById("posDiscount").value) || 0;
    const discountType = document.getElementById("posDiscountType").value;
    const discount = Math.min(discountType === "percent" ? subtotal * (discountRaw / 100) : discountRaw, subtotal);
    ...
    try {
        ...
        const batch = db.batch();
        batch.set(saleRef, {
            items: state.cart.map(l => ({ productId: l.productId, name: l.name, sku: l.sku || "", qty: l.qty,
...
function openReceipt(saleId) {
    ...
    if (voidBtn) {
        ...
        voidBtn.onclick = () => {
            openConfirm("Void this sale?", "This marks the sale as voided for audit purposes.", async () => {
                try {
                    await db.collection("sales").doc(s.id).update({ voided: true, voidedBy: state
                            .currentUser.uid, voidedAt: firebase.firestore.FieldValue
                        .serverTimestamp() });
                    await logActivity("Voided sale", `#${s.id.slice(0,6).toUpperCase()}`);
                    showToast("Sale voided.", "success");
                    bootstrap.Modal.getInstance(document.getElementById("receiptModal")).hide();
                } catch (err) { showToast("Could not void sale: " + err.message, "error"); }
            });
        };
    }
                    ...
                    try {
                        ...
                        (s.items || []).forEach(item => {
                            const product = state.products.find(p => p.id === item
                                .productId);
                            if (product) {
                                const ref = db.collection("products").doc(item
                                .productId);
                                batch.update(ref, { stock: product.stock + item
                                    .qty });
                                const logRef = db.collection("stockLogs").doc();
                                batch.set(logRef, {
                        ...
                        await batch.commit();
                        await logActivity("Refunded sale", `#${s.id.slice(0,6).toUpperCase()}`);
                        showToast("Sale refunded and stock restored.", "success");
                        bootstrap.Modal.getInstance(document.getElementById("receiptModal"))
                            .hide();
                    } catch (err) { showToast("Could not refund: " + err.message,
                        "error"); }
                });
        };
    }
...
function renderStockLogs() {
    try {
        ...
        const typeBadge = { in: "stock-ok", out: "stock-low", damage: "stock-out", sale: "stock-low" };
...
// ============================================================
// 15. REPORTS (with BST‑aware default and range)
// ============================================================
function getReportPeriod() {
    let from = state.financialDateRange.from;
    let to = state.financialDateRange.to;
    if (!from || !to) {
        // Use BST‑aware default (same as dashboard)
        const def = getDefaultRevenueRange();
        from = def.from;
        to = def.to;
    }
...
function renderReports() {
    try {
        ...
        // Damaged inventory is a real cost, so it must reduce profit — not
        // just show up in the separate "Total Loss" figure.
        ...
        sales.forEach(s => (s.items || []).forEach(i => {
            if (!productMap[i.name]) productMap[i.name] = { units: 0, revenue: 0, cogs: 0, profit: 0 };
        ...
        sales.forEach(sale => {
            (sale.items || []).forEach(item => {
                ...
                if (!catMap[cat]) catMap[cat] = { units: 0, revenue: 0, cogs: 0, profit: 0 };
...
function renderInvestmentBreakdown(periodInvestment) {
    try {
        ...
        const items = state.products
            .filter(p => (p.cost || 0) > 0 && (p.stock || 0) > 0)
            .sort((a, b) => ((b.cost || 0) * (b.stock || 0)) - ((a.cost || 0) * (a.stock || 0)))
            .slice(0, 20);
        ...
        const totalInvestment = typeof periodInvestment === 'number' ?
            periodInvestment :
            calcPeriodInvestment(state.financialDateRange.from, state.financialDateRange.to);
        tbody.innerHTML = items.map(p => {
            const inv = (p.cost || 0) * (p.stock || 0);
            const pct = totalInvestment > 0 ? (inv / totalInvestment * 100) : 0;
            return `<tr>
                <td class="text-truncate" style="max-width:150px;" title="${escapeHtml(p.name)}">${escapeHtml(p.name)}</td>
                <td class="num mono">${p.stock}</td>
                <td class="num mono">${fmtMoney(p.cost)}</td>
                <td class="num mono">${fmtMoney(inv)} <span class="text-muted small">(${pct.toFixed(1)}%)</span></td>
              </tr>`;
        }).join('');
    } catch (e) { console.warn("renderInvestmentBreakdown error:", e); }
}
...
function openEditUser(uid) {
    ...
    document.getElementById("removeUserBtn").style.display = uid === state.currentUser.uid ? "none" : "inline-block";
...
function playScanBeep() {
    try {
        ...
        osc2.type = 'sine';
        gain2.gain.setValueAtTime(0.25, audioCtx.currentTime + 0.02);
        ...
        gainNoise.gain.setValueAtTime(0.15, audioCtx.currentTime + 0.02);
        gainNoise.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + 0.06);
...
function exportSalesCSV() {
    ...
    const headers = ["Date", "Receipt", "Cashier", "Items", "Subtotal", "Discount", "Total", "Payment", "Customer"];
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    currencyObserver.observe(document.getElementById("settingCurrency"), { attributes: true, childList: true,
        subtree: true });

    // Dashboard date filter (shares state.financialDateRange with Reports)
    const dashFrom = document.getElementById("dashDateFrom");
    const dashTo = document.getElementById("dashDateTo");
    const dashApply = document.getElementById("dashApplyBtn");
    const dashReset = document.getElementById("dashResetBtn");

    // Initialize the shared default date range (and sync both Dashboard and
    // Reports date inputs) before the very first render of either view.
    ...
    dashApply.addEventListener("click", () => {
        ...
        // Keep the Reports date inputs in sync since the range is shared.
        const repFrom = document.getElementById("reportDateFrom");
        const repTo = document.getElementById("reportDateTo");
    ...
    document.getElementById("deleteProductBtn").addEventListener("click", () => {
        ...
        openConfirm("Delete product?", `Delete "${p.name}" permanently?`, async () => {
            try { await db.collection("products").doc(id).delete();
                ...
                bootstrap.Modal.getInstance(document.getElementById("productModal")).hide(); } catch (
    ...
    document.getElementById("stockLogForm").addEventListener("submit", async (e) => {
        ...
        if (isPerUnit) {
            ...
            const bulkTotal = parseFloat(document.getElementById("stockLogBulkTotal").value) || 0;
            const calculatedUnit = parseFloat(document.getElementById("stockLogBulkUnitPrice").value) || 0;
        ...
        if (newStock < 0) { errEl.textContent = `Not enough stock. Only ${previousStock} units available.`;
            errEl.style.display = "block"; return; }
        ...
        try {
            ...
            batch.set(logRef, {
                ...
                bulkTotal: isPerUnit ? null : parseFloat(document.getElementById("stockLogBulkTotal")
                    .value) || 0,
            });

            await batch.commit();
            await logActivity("Stock movement", `${type.toUpperCase()} ${qty} × ${product.name} @ ${fmtMoney(unitPrice)}`);
    ...
    document.getElementById("addUserForm").addEventListener("submit", async (e) => {
        ...
        try {
            ...
            await db.collection("users").doc(cred.user.uid).set({ name, email, role, createdAt: firebase
                    .firestore.FieldValue.serverTimestamp(), createdBy: state.currentUser.uid });
    ...
    document.getElementById("removeUserBtn").addEventListener("click", () => {
        ...
        openConfirm("Remove account access?", `Delete ${u.name}'s profile?`, async () => {
            try { await db.collection("users").doc(uid).delete();
                ...
                bootstrap.Modal.getInstance(document.getElementById("editUserModal")).hide(); } catch (
    ...
    document.getElementById("settingsForm").addEventListener("submit", async (e) => {
        ...
        const payload = {
            ...
            lowStockThreshold: parseInt(document.getElementById("settingThreshold").value, 10) || 0,
            updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
            updatedBy: state.currentUser.uid,
        };
        try {
    ...
    // Reports date range – use BST. Shares state.financialDateRange with
    // Dashboard, so applying a filter here also refreshes the Dashboard KPI.
    document.getElementById("reportApplyBtn").addEventListener("click", () => {
        ...
        // Keep the Dashboard date inputs in sync since the range is shared.
    ...
    // Same reset sequence as the Dashboard Reset button — this is what keeps
    // the two views from ever drifting apart again.
